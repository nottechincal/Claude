"""
KebabaLab AI Hub — Main FastAPI Application
...
async def lifespan(app: FastAPI):
    logger.info("🌯 KebabaLab AI Hub starting up...")
    await init_db()
    logger.info("✅ Database initialised")
    logger.info(f"📡 Base URL: {settings.base_url}")
    logger.info("🤖 Claude AI: claude-opus-4-6 (ordering) / claude-haiku-4-5 (voice streaming)")
    yield
    logger.info("👋 KebabaLab AI Hub shutting down")


# ─── App ──────────────────────────────────────────────────────────────────────

app = FastAPI(
    title="KebabaLab AI Hub",
